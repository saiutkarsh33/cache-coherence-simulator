...
public:
  L1CacheMESI(int size_b, int assoc, int block_b)
    ...
    sets.reserve(S);
    for (int i=0;i<S;i++) sets.emplace_back(A);
  }
  ...
  CacheLine* find(int idx, uint32_t tag, int& way) {
    ...
    for (int w=0; w<A; w++) {
      auto& ln = v[w];
      if (ln.valid && ln.tag==tag) { way=w; return &v[w]; }
    }
    way = -1; return nullptr;
  }

  // Victim (invalid first, else LRU)
  CacheLine* victim(int idx, int& way) {
    auto& v = sets[idx].ways;
    for (int w=0; w<A; w++) if (!v[w].valid) { way=w; return &v[w]; }
    int vw=0; uint64_t best=v[0].lru;
    for (int w=1; w<A; w++) if (v[w].lru < best) { best=v[w].lru; vw=w; }
    way=vw; return &v[vw];
  }
  ...
  SnoopResult on_busrd(uint32_t addr, uint64_t now) {
    int idx; uint32_t tag; decode(addr, idx, tag);
    int w; auto* ln = find(idx, tag, w);
    SnoopResult r{};
    if (!ln) return r;
    r.had_line = true;
  ...
  SnoopResult on_busrdx(uint32_t addr, uint64_t now) {
    ...
    // All S/E/M -> I
    ln->valid = false; ln->dirty = false; ln->st = MESI::I; ln->lru = now;
    r.invalidated = true;
    return r;
  }
  ...
  Access pr_access(uint64_t tick, bool store, uint32_t addr, int& service_extra_cycles,
    ...
    if (ln) {
      ln->lru = access_clock;
      if (!store) return {true,false,false,BusOp::None,true,false};
      // store hit
      if (ln->st == MESI::M) { ln->dirty=true; return {true,false,true,BusOp::None,true,false}; }
      if (ln->st == MESI::E) { ln->st=MESI::M; ln->dirty=true; return {true,false,true,BusOp::None,true,false}; }
      if (ln->st == MESI::S) {
    ...
    // Fill line
    vic->valid = true; vic->tag = tag; vic->lru = access_clock;
    if (!store) { vic->st = MESI::E; vic->dirty=false; }
    else        { vic->st = MESI::M; vic->dirty=true;  }
