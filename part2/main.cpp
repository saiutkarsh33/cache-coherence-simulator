...
static inline bool file_exists(const std::string& p) {
  struct stat sb{}; return ::stat(p.c_str(), &sb) == 0 && S_ISREG(sb.st_mode);
}
...
static std::vector<std::string> resolve_four(const std::string& input) {
  ...
  bool ok = true;
  for (int i=0;i<4;i++) ok = ok && file_exists(tries[i]);
  if (ok) return tries;
...
int main(int argc, char* argv[]) {
  ...
  bool json = false; for (int i=6;i<argc;i++) if (std::string(argv[i])=="--json") json = true;
