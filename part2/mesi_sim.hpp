...
public:
  ...
  void load_traces(const std::vector<std::string>& paths) {
    if (paths.size()!=4) { std::cerr<<"need 4 traces\n"; std::exit(2); }
    tr.resize(4);
    for (int c=0;c<4;c++) tr[c] = parse_trace(paths[c]);
  }
  ...
  void run() {
    ...
    int live=0; for (int c=0;c<4;c++) if (cur_idx[c] < tr[c].size()) live++;

    while (live>0) {
      int next_core = -1;
      uint64_t next_time = UINT64_MAX;
      for (int c=0;c<4;c++) {
        if (cur_idx[c] >= tr[c].size()) continue;
        uint64_t t = ready_at[c] + tr[c][cur_idx[c]].gap;
        if (t < next_time) { next_time=t; next_core=c; }
      }
      if (next_core<0) break; // done
      ...
      if (acc.hit) {
        ...
        if (acc.needs_bus) {
          ...
          BusTxn t{};
          t.op = acc.busop;
          t.addr = item.addr; t.src_core = c;
          if (t.op == BusOp::BusUpgr) {
            // address-only
            t.data_bytes = 0; t.duration = 1;
            ...
            for (int k=0;k<4;k++) if (k!=c) caches[k].on_busupgr(item.addr, end);
      ...
      cur_idx[c]++;
      if (cur_idx[c] == tr[c].size()) { live--; }
    }

    // Final exec times
    uint64_t max_t=0; for (int c=0;c<4;c++) { st[c].exec_cycles = ready_at[c]; if (ready_at[c]>max_t) max_t=ready_at[c]; }
    overall_exec = max_t;
  }
...
private:
  ...
  static std::vector<TraceItem> parse_trace(const std::string& path) {
    ...
    while (in >> la >> va) {
      int lab = 0;
      try { lab = std::stoi(la); } catch (...) { std::cerr<<"Bad label in "<<path<<"\n"; std::exit(2); }
      if (lab == 2) {
        // compute gap
        uint64_t c = 0;
        if (va.size()>2 && (va[0]=='0' && (va[1]=='x'||va[1]=='X'))) c = std::stoull(va, nullptr, 16);
        else c = std::stoull(va, nullptr, 0);
        gap += c;
      } else {
        // memory access
        uint64_t addr64 = 0;
        if (va.size()>2 && (va[0]=='0' && (va[1]=='x'||va[1]=='X'))) addr64 = std::stoull(va, nullptr, 16);
        else addr64 = std::stoull(va, nullptr, 0);
        TraceItem it; it.gap = gap; it.is_mem = true; it.is_store = (lab==1); it.addr = (uint32_t)addr64;
        out.push_back(it);
        gap = 0;
      }
    }
